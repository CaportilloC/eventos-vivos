import { Component, DestroyRef, inject, signal, OnInit, LOCALE_ID } from '@angular/core';
    ...
    <div class="page-container">
      ...
      <div class="page-header">
        <div class="header-left">
          ...
          <div>
            <h1>Editar Evento</h1>
            <p class="page-subtitle">Modifique los datos del evento seleccionado</p>
          </div>
        </div>
      </div>
...
export class EventEditComponent implements OnInit {
  ...
  protected readonly loading = toSignal(this.eventsFacade.selectedLoading$, { initialValue: false });
  ...
  ngOnInit(): void {
    ...
    this.eventsFacade.selected$
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((evt) => {
        if (evt) this.patchEventForm(evt);
      });
  }
  ...
  private patchEventForm(evt: EventResponse): void {
    const startParsed = parseColombiaDateTimeOffset(evt.startsAt);
    const endParsed = parseColombiaDateTimeOffset(evt.endsAt);
    ...
    this.eventForm.patchValue({
      ...
      venueId: evt.venueId,
      type: evt.type,
      startsAtDate: startParsed?.date ?? null,
      startsAtTime: startParsed?.time ?? '',
      endsAtDate: endParsed?.date ?? null,
      endsAtTime: endParsed?.time ?? '',
  ...
  protected onSubmit(): void {
    ...
    this.eventsFacade.updateEvent(this.eventId, {
      id: this.eventId,
      title: value.title!,
